...
#### Dense 임베딩 + FAISS (`embedder.py`) — 레거시(오프라인)

> 초기 RAG 유사검색은 `embedder.py`의 **MiniLM 384-dim 임베딩 + FAISS `IndexFlatIP`**(L2 정규화 후 내적 = 코사인)로 구현했다. 현재 런타임은 [**pgvector(Gemini 1536-dim)**](#db-스키마--임베딩-적재-파이프라인)로 이전되어 `embedder.py`·`faiss_index.joblib`은 오프라인 레거시로만 남아 있다.

**왜 pgvector로 이전했나**

| | 레거시 (`embedder.py`) | 현재 (pgvector) |
|---|---|---|
| 임베딩 | MiniLM 384-dim (sentence-transformers) | Gemini `gemini-embedding-001` 1536-dim |
| 인덱스 | FAISS `IndexFlatIP` (메모리 상주) | pgvector **HNSW** (DB 영속) |
| 입력 | `question_text + explanation` | `doc_builder.build_doc` (5섹션 SSOT) |
| 신규 문항 | 전체 재인덱싱 필요 | `question_embeddings` upsert만 → 즉시 검색 대상 |
| 검색 | 서버 메모리 FAISS | SQL `cosine(<=>)` |

FAISS는 인덱스가 메모리에 상주하고 신규 문항마다 재빌드가 필요한 반면, pgvector는 임베딩을 DB에 영속화하고 `content_hash` 스테일 검출로 **변경분만 증분 임베딩**한다(한글 경로 미지원 회피를 위해 FAISS 인덱스는 `serialize_index` → joblib bytes로 저장했었다).
...
Linear(128 → 591)
      ↓
Sigmoid → P(correct) for each question (591개)
```
...
총 **591문제** (원본 591 + SQLD 모의고사 55~60회 294문항) — `question_type` 16종.
