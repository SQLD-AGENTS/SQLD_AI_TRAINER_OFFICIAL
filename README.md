...
`IndexFlatIP`는 근사 없이 모든 벡터와 완전 탐색한다. 데이터셋이 591개로 작아 속도 문제가 없으므로 IVF(역색인) 등 근사 인덱스보다 정확도 우선으로 선택했다.

**인덱스 구축 흐름**

```
question_text + explanation (591개)
        ↓
SentenceTransformer.encode()  → ndarray (591, 384)
        ↓
faiss.normalize_L2()           → 각 벡터를 단위 벡터로 정규화
        ↓
faiss.IndexFlatIP(dim=384)
index.add(embeddings)          → 591개 벡터 등록
```
...
scores = embeddings @ query_vec                            # 전체 문제 유사도 (591,)
```
